    ...
    jsMealPanes.forEach(pane => {
        ...
        if (id === 'middag') {
            pane.innerHTML = `
                ...
                <select class="form-select c-input-hand js-main-select"><option value="Geen">-- Geen --</option><option value="Dagmenu">Dagmenu</option><option value="Vegetarisch">Vegetarisch</option></select></div>
...
const validate = (pane, target) => {
    ...
    // 4. DE STRIKTE REGEL: Totaal beleg <= Brood
    const max = parseInt(amts[0].value);
    let h = parseInt(amts[1].value);
    let z = parseInt(amts[2].value);

    if (h + z > max) {
        if (target.dataset.idx == "1") { // Gebruiker verhoogt Hartig
            amts[1].value = Math.min(h, max);
            amts[2].value = max - amts[1].value;
        } else if (target.dataset.idx == "2") { // Gebruiker verhoogt Zoet
            amts[2].value = Math.min(z, max);
            amts[1].value = max - amts[2].value;
        } else { // Brood werd verlaagd
            amts[1].value = Math.min(h, max);
            amts[2].value = Math.max(0, max - amts[1].value);
        }
    }

    // 5. Automatisch naar 'Geen' als aantal 0 wordt door de correctie
    if(amts[1].value == 0) sels[1].value = 'Geen', amts[1].disabled = true;
    if(amts[2].value == 0) sels[2].value = 'Geen', amts[2].disabled = true;
};
// #endregion

// #region *** SAVE & SHOW ***
const save = () => {
    const nr = jsKamerInput.value;
    if (!nr) return alert("Kamer?");
    orders[nr] = {
        ontbijt: getSummary('ontbijt'),
        middag: getSummary('middag'),
        avond: getSummary('avond')
    };
...
const getSummary = (id) => {
    ...
    if(id === 'middag') return `${p.querySelector('.js-main-select').value} (+ ${p.querySelector('.js-dessert-select').value})`;
    const items = Array.from(p.querySelectorAll('.js-sel')).map((s, i) => ({
        val: s.value, qty: p.querySelectorAll('.js-amt')[i] ? p.querySelectorAll('.js-amt')[i].value : 0
    }));
    let s = items.filter(i => i.val !== 'Geen').map(i => `${i.qty}x ${i.val}`).join(', ');
    if(p.querySelector('.js-boter').checked) s += " + Boter";
    return s || "---";
};

document.addEventListener('DOMContentLoaded', () => {
    initForms();
    jsSubmitBtn.onclick = save;
    // Overzicht logica hier...
});
// #endregion